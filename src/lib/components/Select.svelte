...
<script lang="ts">
	...
	let open = false;
	function enter(node: HTMLElement, { delay = 0, duration = 100 } = {}) {
	...
	async function handleKeyPress(e: KeyboardEvent) {
		switch (e.code) {
			...
			case 'Space':
				e.preventDefault();
				if (active) return await selectActive();
				toggle();
				break;
			default:
				return;
...
<div class="wrap">
	<div class="inner" class:fluid>
		...
		>
			<span class="value">{typeof value === 'string' ? value : value?.name ?? ''}</span>
			<input
				{name}
				{disabled}
				on:blur={close}
