<script lang="ts">
	...
	type FocusNode = HTMLElement & { _prevTabindex?: string | null };
	let nonModalNodes: NodeListOf<FocusNode>;

	function lockFocus() {
		const modalNodes = Array.from(document.querySelectorAll('div[data-lock] *')) as FocusNode[];
...
<style>
	...
	div.footer {
		...
		flex-shrink: 0;
		overflow: hidden;
		transition: height 0.2s cubic-bezier(0.47, 0, 0.745, 0.715), box-shadow 0.2s ease-in;
	}
